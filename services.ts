...
export const tmdbService = {
  ...
  discoverMedia: async (params: Record<string, string> = {}, mediaType: 'movie' | 'tv' = 'movie'): Promise<TMDBListResponse<MediaItem>> => {
    const endpoint = `/discover/${mediaType}`;
    const response = await fetchTMDB<TMDBListResponse<any>>(endpoint, { ...params, include_adult: 'false', language: 'en-US', sort_by: 'popularity.desc' });
    response.results = response.results.map(item => ({
        ...item,
